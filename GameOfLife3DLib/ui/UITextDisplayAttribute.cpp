...
ui::UITextDisplayAttribute::UITextDisplayAttribute(void)
...
HRESULT
ui::UITextDisplayAttribute::GetDisplayAttributeProperties(OUT DisplayAttributeProperties &displayAttributeProperties) {
...
HRESULT ui::UITextDisplayAttribute::GetDisplayAttributeTrackPropertyRange(
  ...
  ULONG ulNumProp = static_cast<ULONG>(displayAttributeProperties.Count());
  if (!ulNumProp) {
    return E_FAIL;
  }
  ...
  hr = pTfContext->TrackProperties(ppguidProp, ulNumProp, 0, 0, &pProp);
  if (SUCCEEDED(hr)) {
    *ppProp = pProp;
  }
...
HRESULT ui::UITextDisplayAttribute::GetDisplayAttributeData(IN TfEditCookie editCookie, IN ITfReadOnlyProperty *pProp,
                                                            IN ITfRange *pRange,
...
HRESULT ui::UITextDisplayAttribute::GetAttributeColor(IN const TF_DA_COLOR &daColor, OUT COLORREF &colorref) {
  switch (daColor.type) {
  case TF_CT_SYSCOLOR:
