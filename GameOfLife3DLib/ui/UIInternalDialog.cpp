    ...
    m_titleBackgroundBrush(std::make_shared<graphics::brush::LinearGradientBrush>())
{
    ...
    m_outerShineBrush->AddGradientStop(
        D2D1::GradientStop(1.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.35f, 1.0f)));

    m_innerShadowBrush->AddGradientStop(
        D2D1::GradientStop(0.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.18f, 1.0f)));
    m_innerShadowBrush->AddGradientStop(
        D2D1::GradientStop(1.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.18f, 1.0f)));

    m_backgroundBrush->AddGradientStop(
        D2D1::GradientStop(0.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.25f, 1.0f)));
    m_backgroundBrush->AddGradientStop(
        D2D1::GradientStop(1.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.25f, 1.0f)));

    m_titleForegroundBrush->AddGradientStop(
        D2D1::GradientStop(0.0, D2D1::ColorF(D2D1::ColorF::White)));
    m_titleForegroundBrush->AddGradientStop(
        D2D1::GradientStop(1.0, D2D1::ColorF(D2D1::ColorF::White)));
...
HRESULT ui::UIInternalDialog::CreateDeviceDependentResources(
...
HRESULT ui::UIInternalDialog::Render(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget )
{
    pRenderTarget->FillRectangle(
        D2D1::RectF(0, 0, GetWidth(), GetHeight()),
        m_outerShineBrush->GetBrush());
    pRenderTarget->FillRectangle(
        D2D1::RectF(1.0f, 1.0f, GetWidth() - 1.0f, GetHeight() - 1.0f),
        m_innerShadowBrush->GetBrush());
    pRenderTarget->FillRectangle(
        D2D1::RectF(2.0f, 2.0f, GetWidth() - 2.0f, GetHeight() - 2.0f),
        m_backgroundBrush->GetBrush());

    pRenderTarget->FillRectangle(
        D2D1::RectF(2.0f, 2.0f, GetWidth() - 2.0f, m_textMetrics.height + 2.0f + TEXT_MARGIN),
        m_titleBackgroundBrush->GetBrush());
    pRenderTarget->DrawTextLayout(
        D2D1::Point2F(2.0f + TEXT_MARGIN, 2.0f), m_textLayout,
        m_titleForegroundBrush->GetBrush());

    D2D1_MATRIX_3X2_F origMatrix;
    pRenderTarget->GetTransform(&origMatrix);
    D2D1_MATRIX_3X2_F matrix = origMatrix * D2D1::Matrix3x2F::Translation(0.0f, m_textMetrics.height + 2.0f + TEXT_MARGIN);
