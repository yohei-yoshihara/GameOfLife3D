...
HRESULT ui::UITextField::Initialize( graphics::D3DInteropHelper *pD3DInteropHelper )
{
    ...
    CHK_FATAL_HRESULT(m_pDocMgr->CreateContext(m_tsfManager->GetClientId(),
                      0,
                      static_cast<ITextStoreACP*>(m_textStoreACP),
                      &m_pContext,
                      &m_editCookie));
...
HRESULT ui::UITextField::CreateDeviceIndependentResources(
    graphics::D3DInteropHelper *pD3DInteropHelper )
{
    ...
    CHK_FATAL_HRESULT(pD2DFactory->CreateStrokeStyle(
                          ...
                          nullptr,
                          0,
                          &m_compositionSolidUnderlineStrokeStyle
                      ));
    ...
    CHK_FATAL_HRESULT(pD2DFactory->CreateStrokeStyle(
                          ...
                          nullptr,
                          0,
                          &m_compositionDotUnderlineStrokeStyle));
    ...
    CHK_FATAL_HRESULT(pD2DFactory->CreateStrokeStyle(
                          ...
                          nullptr,
                          0,
                          &m_compositionDashUnderlineStrokeStyle));
    ...
    CHK_FATAL_HRESULT(pD2DFactory->CreateStrokeStyle(
                          ...
                          nullptr,
                          0,
                          &m_compositionSquiggleUnderlineStrokeStyle));
...
HRESULT ui::UITextField::CreateDeviceDependentResources(
    graphics::D3DInteropHelper *pD3DInteropHelper,
...
HRESULT ui::UITextField::_RecreateTextLayout(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget)
{
#ifdef DEBUG_UITEXTFIELD
...
void ui::UITextField::_RenderBackground(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget)
{
#ifdef DEBUG_UITEXTFIELD
    ...
    pRenderTarget->FillRectangle(
        D2D1::RectF(1.0f, 1.0f, GetWidth() - 1.0f, GetHeight() - 1.0f),
        m_backgroundColorBrush);
#ifdef DEBUG_UITEXTFIELD
    LOG_LEAVE(SEVERITY_LEVEL_DEBUG);
#endif
}

void ui::UITextField::_RenderBox(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget)
{
#ifdef DEBUG_UITEXTFIELD
    ...
    pRenderTarget->DrawRectangle(
        D2D1::RectF(1.0f, 1.0f, GetWidth() - 1.0f, GetHeight() - 1.0f),
        m_borderLineColorBrush);
    if (HasFocus()) {
        pRenderTarget->DrawRectangle(
            D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()),
            m_focusBorderLineColorBrush);
    }
#ifdef DEBUG_UITEXTFIELD
    LOG_LEAVE(SEVERITY_LEVEL_DEBUG);
#endif
...
HRESULT ui::UITextField::_RenderText(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget,
                ...
                for (size_t i = 0; i < actualHitTextCount; i++) {
                    ...
                    if (compositionAttribute.displayAttribute.crLine.type != TF_CT_NONE) {
                        ...
                        CHK_FATAL_HRESULT(pRenderTarget->CreateSolidColorBrush(
                                              graphics::color::ColorUtil::ColorrefToColorF(colorref),
                                              &pLineColorBrush));
                    }
...
void ui::UITextField::_RenderHighlight(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget,
    UIRectangle textRect )
{
    ...
    ID2D1RenderTarget *pRenderTarget )
{
    ...
    UIRectangle textRect(
        INNER_MARGIN,
        INNER_MARGIN,
        GetWidth() - INNER_MARGIN * 2,
        GetHeight() - INNER_MARGIN * 2
    );
...
void ui::UITextField::AddCompositionRenderInfo(
    LONG startPos,
    LONG endPos,
...
void ui::UITextField::NotifyTextReplace(
    LONG oldTotalLength,
    LONG startPos,
    LONG replacedTextLength,
...
void ui::UITextField::NotifyTextRemove(
    LONG oldTotalLength,
    LONG startPos,
    LONG removedTextLength )
{
#ifdef DEBUG_UITEXTFIELD
    LOG_ENTER(SEVERITY_LEVEL_DEBUG);
#endif
...
void ui::UITextField::NotifyTextInsert(
    LONG oldTotalLength,
    LONG startPos,
    LONG textToInsertLength )
{
...
HRESULT ui::UITextField::GetTextExt( LONG acpStart, LONG acpEnd, RECT *prc, BOOL *pfClipped )
{
    ...
    *pfClipped = FALSE;
    ...
    UINT32 actualCount;
    if (m_textLayout->HitTestTextRange(acpStart, acpEnd - acpStart + 1, INNER_MARGIN, INNER_MARGIN,
                                       nullptr, 0, &actualCount) != E_NOT_SUFFICIENT_BUFFER) {
        return E_INVALIDARG;
    }
...
#ifdef DEBUG_UITEXTFIELD
    LOG_LEAVE(SEVERITY_LEVEL_DEBUG)
            << L"rect=" << prc->left << L"," << prc->top << L"," << prc->right << L"," << prc->bottom;
#endif
    return S_OK;
}
...
HRESULT ui::UITextField::_CalculateCaretSizeAndPosition(
    ...
    OUT DWRITE_HIT_TEST_METRICS *pHitTestMetrics)
{
    ...
    CHK_FATAL_HRESULT(m_textLayout->HitTestTextPosition(
                          caretPosition,
                          activeSelEnd == TS_AE_END,
                          &(caretPosInTextCoordinate.x),
                          &(caretPosInTextCoordinate.y),
                          pHitTestMetrics));
#ifdef DEBUG_UITEXTFIELD
    LOG_LEAVE(SEVERITY_LEVEL_DEBUG) << L"caretPosition = " << caretPosition
                                    << L", isTrailingHit = " << std::boolalpha << (activeSelEnd == TS_AE_END)
                                    << ", caretPosInModel = " << caretPosInTextCoordinate;
#endif
    return S_OK;
}
...
void ui::UITextField::CalculateSelectionFromDragRect(
    ...
    OUT TsActiveSelEnd* activeSelEnd )
{
    ...
    if (!isStartPosInside) {
        ...
        if (textSize.width <= startX && startX <= m_textDisplayAreaHPos + GetTextDisplayAreaSize().width &&
                0.0f <= startY && startY <= GetTextDisplayAreaSize().height) {
    ...
    *acpStart = caretMetrics.textPosition;
    ...
    if (!isEndPosInside) {
        ...
        if (textSize.width <= endX && endX <= m_textDisplayAreaHPos + GetTextDisplayAreaSize().width &&
                0.0f <= endY && endY <= GetTextDisplayAreaSize().height) {
            // click the right side of the text
            caretMetrics.textPosition = static_cast<UINT32>(m_textStoreACP->GetTextLength());
        } else {
            *inside = false;
...
void ui::UITextField::OnLeftMouseDown(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UIPoint clientPoint,
    ...
    OUT bool* eaten )
{
    ...
    if (!HasFocus()) {
        ...
        m_textStoreACP->OnMouseEvent(
            clientPoint.x - INNER_MARGIN,
            clientPoint.y - INNER_MARGIN,
            clientPoint.x - INNER_MARGIN,
            clientPoint.y - INNER_MARGIN);
    }
    UpdateCaret();
...
void ui::UITextField::OnDraggingStart(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    ...
    OUT bool* eaten )
{
    ...
    m_textStoreACP->OnMouseEvent(
        clientDragRect.start.x - INNER_MARGIN,
        clientDragRect.start.y - INNER_MARGIN,
        clientDragRect.end.x - INNER_MARGIN,
        clientDragRect.end.y - INNER_MARGIN);
    *eaten = true;
#ifdef DEBUG_UITEXTFIELD
    LOG_LEAVE(SEVERITY_LEVEL_DEBUG);
#endif
...
void ui::UITextField::OnDragging(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UI2Points clientDragRect,
...
void ui::UITextField::OnDraggingEnd(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UI2Points clientDragRect,
...
void ui::UITextField::OnSetFocus(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    ULONGLONG timestampInMilliSeconds,
...
void ui::UITextField::OnKillFocus(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    ULONGLONG timestampInMilliSeconds,
