...
ui::UIButton::UIButton(void)
  ...
  m_fgGradientBrush->AddGradientStop(D2D1::GradientStop(0.0, D2D1::ColorF(D2D1::ColorF::White, 1.0f)));
  m_fgGradientBrush->AddGradientStop(D2D1::GradientStop(1.0, D2D1::ColorF(D2D1::ColorF::White, 1.0f)));

  m_bgGradientBrush->AddGradientStop(D2D1::GradientStop(0.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.35f, 1.0f)));
  m_bgGradientBrush->AddGradientStop(D2D1::GradientStop(1.0, graphics::color::ColorUtil::HsvToRgb(0, 0, 0.10f, 1.0f)));
...
HRESULT ui::UIButton::Initialize(graphics::D3DInteropHelper *pD3DInteropHelper) {
  UNREFERENCED_PARAMETER(pD3DInteropHelper);
  return S_OK;
}

HRESULT ui::UIButton::CreateDeviceDependentResources(graphics::D3DInteropHelper *pD3DInteropHelper,
                                                     ID2D1RenderTarget *pRenderTarget) {
  CHK_FATAL_HRESULT(m_fgGradientBrush->CreateDeviceDependentResource(
      pRenderTarget, D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), 1.0f));
  CHK_FATAL_HRESULT(m_bgGradientBrush->CreateDeviceDependentResource(
      pRenderTarget, D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), 1.0f));
  CHK_FATAL_HRESULT(m_bgDownGradientBrush->CreateDeviceDependentResource(
      pRenderTarget, D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), 1.0f));
  CHK_FATAL_HRESULT(m_bgMouseOverGradientBrush->CreateDeviceDependentResource(
      pRenderTarget, D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), 1.0f));
...
void ui::UIButton::SetForegroundColor(const std::vector<D2D1_GRADIENT_STOP> &foregroundColors,
...
void ui::UIButton::SetBackgroundColor(const std::vector<D2D1_GRADIENT_STOP> &backgroundColors,
...
HRESULT ui::UIButton::Render(graphics::D3DInteropHelper *pD3DInteropHelper, ID2D1RenderTarget *pRenderTarget) {
#ifdef DEBUG_EVENT
  ...
  case BUTTON_STATUS_DOWN:
    pRenderTarget->FillRectangle(D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), m_bgDownGradientBrush->GetBrush());
  ...
  case BUTTON_STATUS_MOUSEOVER:
    pRenderTarget->FillRectangle(D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()),
  ...
  default:
    pRenderTarget->FillRectangle(D2D1::RectF(0.0f, 0.0f, GetWidth(), GetHeight()), m_bgGradientBrush->GetBrush());
    pRenderTarget->DrawLine(p2, p4, m_shadowColorBrush);
    pRenderTarget->DrawLine(p3, p4, m_shadowColorBrush);
    pRenderTarget->DrawLine(p1, p2, m_shineColorBrush);
    pRenderTarget->DrawLine(p1, p3, m_shineColorBrush);
  ...
  pRenderTarget->DrawText(
      m_text.c_str(), static_cast<UINT>(m_text.size()), m_textFormat,
      D2D1::RectF((GetWidth() - textWidth) / 2.0f, (GetHeight() - textHeight) / 2.0f, GetWidth(), GetHeight()),
...
void ui::UIButton::OnLeftMouseDown(HWND hWnd, WPARAM wParam, LPARAM lParam, UIPoint clientPoint,
...
#endif
  if (0.0f <= clientPoint.x && clientPoint.x <= GetWidth() && 0.0f <= clientPoint.y && clientPoint.y <= GetHeight()) {
...
void ui::UIButton::OnLeftMouseUp(HWND hWnd, WPARAM wParam, LPARAM lParam, UIPoint clientPoint,
                                 ULONGLONG timestampInMilliSeconds, OUT bool *eaten) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
...
void ui::UIButton::OnMouseOver(HWND hWnd, WPARAM wParam, LPARAM lParam, UIPoint clientPoint, UIDelta delta,
                               ULONGLONG timestampInMilliSeconds, OUT bool *eaten) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
  ...
  UNREFERENCED_PARAMETER(timestampInMilliSeconds);
#ifdef DEBUG_EVENT
  LOG_ENTER(SEVERITY_LEVEL_DEBUG) << L"text = " << m_text << L", clientPoint = " << clientPoint << L", delta = "
                                  << delta;
#endif
  if (0.0f <= clientPoint.x && clientPoint.x <= GetWidth() && 0.0f <= clientPoint.y && clientPoint.y <= GetHeight()) {
...
void ui::UIButton::OnMouseOut(HWND hWnd, WPARAM wParam, LPARAM lParam, UIPoint clientPoint, UIDelta delta,
                              ULONGLONG timestampInMilliSeconds, OUT bool *eaten) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
