...
HRESULT ui::UIRoot::CreateDeviceDependentResources(
...
HRESULT ui::UIRoot::Render( graphics::D3DInteropHelper *pD3DInteropHelper, ID2D1RenderTarget *pRenderTarget )
{
    ...
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
        ...
        if (m_dialogs.count(m_displayingDialogName) == 0) {
            LOG(SEVERITY_LEVEL_ERROR) << L"could not found the internal dialog which name is '"
...
void ui::UIRoot::OnSetFocus( HWND hWnd, WPARAM wParam, LPARAM lParam, ULONGLONG timestampInMilliSeconds, OUT bool* eaten )
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
...
void ui::UIRoot::OnKillFocus( HWND hWnd, WPARAM wParam, LPARAM lParam, ULONGLONG timestampInMilliSeconds, OUT bool* eaten )
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
...
void ui::UIRoot::OnLeftMouseDown(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UIPoint clientPoint,
    ...
    OUT bool* eaten )
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
        ...
        dialog->OnLeftMouseDown(
            ...
            wParam,
            lParam,
            dialog->Translate(clientPoint, GetInsets().left, GetInsets().top),
            timestampInMilliSeconds,
            eaten);
    }
}

void ui::UIRoot::OnLeftMouseUp(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UIPoint clientPoint,
...
void ui::UIRoot::OnDraggingStart(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    ...
    OUT bool* eaten )
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
        ...
        dialog->OnDraggingStart(
            ...
            wParam,
            lParam,
            dialog->Translate(clientDragRect, GetInsets().left, GetInsets().top),
            delta,
            timestampInMilliSeconds,
            eaten);
    }
...
void ui::UIRoot::OnRightSingleClick(
    HWND hWnd,
    WPARAM wParam,
    LPARAM lParam,
    UIPoint clientPoint,
    ...
    OUT bool* eaten)
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
        ...
        dialog->OnZoom(
            ...
            dialog->Translate(clientPoint, GetInsets().left, GetInsets().top),
            pGestureInfo,
            dialog->Translate(zoomCenterPoint, GetInsets().left, GetInsets().top),
            zoomFactor,
            timestampInMilliSeconds,
            eaten);
    }
    ...
    OUT bool* eaten)
{
    if (m_displayingDialogName.length() == 0 || m_dialogs.count(m_displayingDialogName) == 0) {
        ...
        dialog->OnRotate(
            ...
            dialog->Translate(clientPoint, GetInsets().left, GetInsets().top),
            pGestureInfo,
            dialog->Translate(rotateCenterPoint, GetInsets().left, GetInsets().top),
            rotateAngle,
            timestampInMilliSeconds,
            eaten);
    }
