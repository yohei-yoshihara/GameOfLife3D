...
namespace ui {
  ...
  public:
    ...
    UIPoint(FLOAT _x, FLOAT _y)
        : x(_x)
        , y(_y) {
    }
    explicit UIPoint(POINT p)
  ...
  public:
    ...
    explicit UIRectangle(RECT r)
        : x(static_cast<FLOAT>(r.left))
        , y(static_cast<FLOAT>(r.top))
        , width(static_cast<FLOAT>(r.right - r.left + 1))
  ...
  public:
    ...
    UI2Points(RECT rect)
        : start(static_cast<FLOAT>(rect.left), static_cast<FLOAT>(rect.top))
  ...
  inline std::wostream &operator<<(std::wostream &os, D2D1_MATRIX_3X2_F const &matrix) {
    os << L"D2D1_MATRIX_3X2_F[" << matrix._11 << L"," << matrix._12 << L"/" << matrix._21 << L"," << matrix._22 << L"/"
