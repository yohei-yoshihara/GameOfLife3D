...
namespace gameOfLife3D {
  namespace dialogs {
    enum VERTEX_GENERATION_METHOD { VERTEX_GENERATION_METHOD_CPU = 0, VERTEX_GENERATION_METHOD_GPU = 1 };
    ...
    public:
      ...
      bool operator==(const SettingData &settingData) const {
        return width == settingData.width && height == settingData.height && generations == settingData.generations
               && updateInterval == settingData.updateInterval
               && vertexGenerationMethod == settingData.vertexGenerationMethod;
      }
