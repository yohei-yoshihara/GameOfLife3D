...
HRESULT gameOfLife3D::draw3d::Draw3DWithGS::CreateD3DResources(graphics::D3DInteropHelper *pD3DInteropHelper,
  ...
  XMFLOAT3 p0;
  XMFLOAT3 p1;
  gameOfLife3D::draw3d::Draw3DUtil::CalculateP0P1(width, height, m_maxNumberOfGenerations, p0, p1);

  XMFLOAT3 cubeLength;
  XMFLOAT3 cubeMargin;
  gameOfLife3D::draw3d::Draw3DUtil::CalculateLengthAndMargin(width, height, m_maxNumberOfGenerations, p0, p1,
                                                             m_marginRate, cubeLength, cubeMargin);
  ...
  CHK_FATAL_HRESULT(pD3DInteropHelper->CompileShaderFromResource(MAKEINTRESOURCE(IDR_SHADER_SIMPLE_D3D_GS), "VS",
                                                                 "vs_4_0", &pVSBlob));
  CHK_FATAL_HRESULT(
  ...
  CHK_FATAL_HRESULT(pD3DInteropHelper->CompileShaderFromResource(MAKEINTRESOURCE(IDR_SHADER_SIMPLE_D3D_GS), "GS",
                                                                 "gs_4_0", &pGSBlob));
  CHK_FATAL_HRESULT(pDevice->CreateGeometryShader(pGSBlob->GetBufferPointer(), pGSBlob->GetBufferSize(), nullptr,
                                                  &m_pGeometryShader));

  // Pixel Shader
  CComPtr<ID3DBlob> pPSBlob = nullptr;
  CHK_FATAL_HRESULT(pD3DInteropHelper->CompileShaderFromResource(MAKEINTRESOURCE(IDR_SHADER_SIMPLE_D3D_GS), "PS",
                                                                 "ps_4_0", &pPSBlob));
  CHK_FATAL_HRESULT(
...
HRESULT gameOfLife3D::draw3d::Draw3DWithGS::MoveNextGeneration(graphics::D3DInteropHelper *pD3DInteropHelper) {
  UNREFERENCED_PARAMETER(pD3DInteropHelper);
...
HRESULT gameOfLife3D::draw3d::Draw3DWithGS::RecreateSizedResources(UINT nWidth, UINT nHeight,
                                                                   graphics::D3DInteropHelper *pD3DInteropHelper,
                                                                   ID3D11Device *pDevice,
                                                                   ID3D11DeviceContext *pImmeidiateContext) {
...
HRESULT gameOfLife3D::draw3d::Draw3DWithGS::Draw3D(graphics::D3DInteropHelper *pD3DInteropHelper, ID3D11Device *pDevice,
  ...
  for (UINT generation = 0; generation < m_maxNumberOfGenerations; ++generation) {
    CopyMemory(&vertexes[area * generation], &m_lifeDataArray[generation]->front(),
