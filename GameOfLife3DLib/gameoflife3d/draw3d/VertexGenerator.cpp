...
const gameOfLife3D::draw3d::VERTEX_INDEX gameOfLife3D::draw3d::VertexGenerator::s_indices[] = {
    3, 1, 0,
    2, 1, 3,
...
void gameOfLife3D::draw3d::VertexGenerator::_AnalyzeVertices(const std::vector<gameOfLife3D::draw3d::VertexInfo> &vertices, OUT std::vector<XMFLOAT3> &cubes)
{
...
void gameOfLife3D::draw3d::VertexGenerator::UpdateLengthAndMargin()
{
    ...
    if (m_attributeModified) {
        Draw3DUtil::CalculateLengthAndMargin(
            m_width, m_height, m_maxNumberOfGenerations, m_p0, m_p1, m_marginRate,
            m_cubeLength, m_cubeMargin);
    }
    m_attributeModified = false;
}

XMFLOAT3 gameOfLife3D::draw3d::VertexGenerator::CalculateCubeCenterPosition(
    const XMFLOAT3 &cubeLength,
    const XMFLOAT3 &cubeMargin,
...
void gameOfLife3D::draw3d::VertexGenerator::CheckVertexAndIndex(
    ...
    IN UINT numberOfIndexes )
{
    ...
    if (nCubes_v != nCubes_i) {
        LOG(SEVERITY_LEVEL_ERROR) << L"invalid number of cubes, nCubes_v = " << nCubes_v
                                  << L", nCubes_i = " << nCubes_i;
    }
    for (UINT i = 0; i < nCubes_v; ++i) {
        ...
        for (UINT j = 0; j < NUMBER_OF_INDEXES_PER_LIFE; ++j) {
            ...
            if (index < startIndex || endIndex < index) {
                LOG(SEVERITY_LEVEL_ERROR) << L"index is out of range, index = " << index
                                          << L", startIndex = " << startIndex << L", endIndex = " << endIndex;
            }
        }
    }
}
