...
const wchar_t *COLORSETDEF = L"solid[solid1](rgba(1.0,0.0,0.0,1.0)),"
...
HRESULT gameOfLife3D::CanvasPanel::Initialize(std::weak_ptr<MainWnd> mainWnd) {
  ...
  m_hwnd = CreateWindow(L"CanvasPanel", L"CanvasPanel",
                        WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN /*| WS_HSCROLL | WS_VSCROLL*/, rc.left, rc.top,
                        rc.right, rc.bottom, parentHwnd, nullptr, HINST_THISCOMPONENT, this);
...
HRESULT gameOfLife3D::CanvasPanel::InitializeComponents() {
  ...
  horizontalScrollBar->AddPositionChangedCallback([&](float angle) { NotifyHorizontalScrollbarChanged(angle); });
  ...
  verticalScrollBar->SetAnchor(
      ui::UIAnchor(ui::ANCHOR_TOP | ui::ANCHOR_RIGHT | ui::ANCHOR_BOTTOM, 20.0f, 60.0f, 20.0f, 60.0f));
  verticalScrollBar->SetDirection(ui::UIScrollBar::SCROLLBAR_DIRECTION_VERTICAL);
  verticalScrollBar->AddPositionChangedCallback([&](float angle) { NotifyVerticalScrollbarChanged(angle); });
...
// LOG(SEVERITY_LEVEL_DEBUG) << util::WMessage(util::WMessageArgs(hWnd, message, wParam, lParam));
  ...
  if (message == WM_CREATE) {
    ...
    CanvasPanel *pCanvasPanel
        = reinterpret_cast<CanvasPanel *>(static_cast<LONG_PTR>(::GetWindowLongPtrW(hWnd, GWLP_USERDATA)));
    if (pCanvasPanel) {
...
HRESULT gameOfLife3D::CanvasPanel::CreateDeviceIndependentResources(graphics::D3DInteropHelper *pD3DInteropHelper,
...
HRESULT gameOfLife3D::CanvasPanel::CreateD2DResources(graphics::D3DInteropHelper *pD3DInteropHelper,
  ...
  CHK_FATAL_HRESULT(pD3DInteropHelper->GetDWriteFactory()->CreateTextFormat(
      L"Arial", nullptr, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 16, L"",
      &m_pFpsTextFormat));

  CHK_FATAL_HRESULT(pD3DInteropHelper->GetDWriteFactory()->CreateTextLayout(L"", 0, m_pFpsTextFormat, FLT_MAX, FLT_MAX,
                                                                            &m_pFpsTextLayout));

  CHK_FATAL_HRESULT(pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), D2D1::BrushProperties(),
                                                         &m_pFpsTextBrush));
...
HRESULT gameOfLife3D::CanvasPanel::Draw2D(graphics::D3DInteropHelper *pD3DInteropHelper,
  ...
  if (m_fpsEnabled) {
    ...
    if (updated) {
      ...
      CHK_FATAL_HRESULT(pD3DInteropHelper->GetDWriteFactory()->CreateTextLayout(
          fps.c_str(), static_cast<UINT32>(fps.length()), m_pFpsTextFormat, FLT_MAX, FLT_MAX, &m_pFpsTextLayout));
    }
  }
...
HRESULT gameOfLife3D::CanvasPanel::CreateD3DResources(graphics::D3DInteropHelper *pD3DInteropHelper,
...
HRESULT gameOfLife3D::CanvasPanel::RecreateSizedResources(UINT nWidth, UINT nHeight,
...
HRESULT gameOfLife3D::CanvasPanel::_UpdateConstantBuffer(graphics::D3DInteropHelper *pD3DInteropHelper,
                                                         ID3D11Device *pDevice,
                                                         ID3D11DeviceContext *pImmediateContext) {
  UNREFERENCED_PARAMETER(pD3DInteropHelper);
  UNREFERENCED_PARAMETER(pDevice);
...
HRESULT gameOfLife3D::CanvasPanel::Draw3D(graphics::D3DInteropHelper *pD3DInteropHelper, ID3D11Device *pDevice,
                                          ID3D11DeviceContext *pImmediateContext) {
  UNREFERENCED_PARAMETER(pD3DInteropHelper);
  UNREFERENCED_PARAMETER(pDevice);
  UNREFERENCED_PARAMETER(pImmediateContext);
  ...
  pImmediateContext->IASetIndexBuffer( pIndexBuffer,
                                       sizeof(gameOfLife3D::draw3d::VERTEX_INDEX) == 2 ? DXGI_FORMAT_R16_UINT :
...
void gameOfLife3D::CanvasPanel::InitializeLifeSimulator(graphics::D3DInteropHelper *pD3DInteropHelper) {
  ...
  m_pLifeSimulator = gameOfLife3D::lifeSimulator::LifeSimulatorFactory::Create(
      pD3DInteropHelper->IsComputeShaderSupported() ? gameOfLife3D::lifeSimulator::SIMULATOR_TYPE_COMPUTESHADER
...
INT_PTR CALLBACK DialogProc_CSERROR_DIALOG(__in HWND hwndDlg, __in UINT uMsg, __in WPARAM wParam, __in LPARAM lParam) {
...
INT_PTR CALLBACK DialogProc_CSNOTSUPPORT_DIALOG(__in HWND hwndDlg, __in UINT uMsg, __in WPARAM wParam,
                                                __in LPARAM lParam) {
  UNREFERENCED_PARAMETER(lParam);
  switch (uMsg) {
  case WM_INITDIALOG:
...
void gameOfLife3D::CanvasPanel::OpenCSFile(const std::wstring &fileName) {
  ...
  if (FAILED(hr)) {
    if (hr == E_NOTIMPL) {
      DialogBox(mui::MUI::GetHModule(), MAKEINTRESOURCE(IDD_CSNOTSUPPORT_DIALOG), m_hwnd,
                DialogProc_CSNOTSUPPORT_DIALOG);
    } else {
      DialogBox(mui::MUI::GetHModule(), MAKEINTRESOURCE(IDD_CSERROR_DIALOG), m_hwnd, DialogProc_CSERROR_DIALOG);
    }
  }
}
