...
HRESULT gameOfLife3D::MainWnd::Initialize() {
  ...
  if (hIcon == nullptr) {
    ...
    SPDLOG_ERROR(L"LoadIcon failed with error: {}", lastError);
    return E_FAIL;
  }
  ...
  if (hAccel == nullptr) {
    DWORD lastError = GetLastError();
    SPDLOG_ERROR(L"LoadAccelerators failed with error: {}", lastError);
    return E_FAIL;
  }
  ...
  m_hwnd = CreateWindow(L"MainWnd", mui::MUI::GetString(IDS_APP_NAME), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
                        CW_USEDEFAULT, CW_USEDEFAULT, static_cast<UINT>(std::ceil(1024.f * dpiX / 96.f)),
                        static_cast<UINT>(std::ceil(768.f * dpiY / 96.f)), nullptr, nullptr, HINST_THISCOMPONENT, this);
  HRESULT hr = m_hwnd ? S_OK : E_FAIL;
  if (FAILED(hr)) {
    DWORD lastError = GetLastError();
    SPDLOG_ERROR(L"CreateWindow failed with error: {}", lastError);
  } else {
...
// SPDLOG_DEBUG( << util::WMessage(util::WMessageArgs(hWnd, message, wParam, lParam));
  ...
  if (message == WM_CREATE) {
    ...
    MainWnd *pMainWnd = reinterpret_cast<MainWnd *>(static_cast<LONG_PTR>(GetWindowLongPtr(hWnd, GWLP_USERDATA)));
    ...
    if (pMainWnd) {
      ...
      case WM_COMMAND: {
        ...
        break;
      case WM_SIZE:
        if (pMainWnd->m_canvasPanel.get() != nullptr && pMainWnd->m_canvasPanel->IsInitialized()) {
...
void gameOfLife3D::MainWnd::OnOpenFile() {
  static COMDLG_FILTERSPEC dialogFiltersMM[] = {
      {L"Life File(*.lif;*.life)", L"*.lif;*.life"}, {L"All types(*.*)", L"*.*"},
  };
...
COMDLG_FILTERSPEC DIALOG_FILTERS[] = {
    {L"GameOfLife3D(.l3d)", L"*.l3d"}, {L"Life 1.06(.lif)", L"*.lif"},
};
...
void gameOfLife3D::MainWnd::OnOpenCSFile() {
  static COMDLG_FILTERSPEC dialogFiltersMM[] = {
      {L"effect File(*.fx)", L"*.fx"}, {L"compute shader File(*.cs)", L"*.cs"}, {L"All types(*.*)", L"*.*"},
  };
