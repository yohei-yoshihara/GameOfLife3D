...
HRESULT gameOfLife3D::lifeSimulator::LifeSimulatorCS::Initialize(graphics::D3DInteropHelper *pD3DInteropHelper)
{
    ...
    CHK_FATAL_HRESULT(pD3DInteropHelper->CompileShaderFromResource(
                          MAKEINTRESOURCE(IDR_SHADER_GAMEOFLIFE),
                          "main",
                          pProfile,
                          &pCSBlob));
    CHK_FATAL_HRESULT(m_pDevice->CreateComputeShader(
    ...
    for (UINT bufIndex = 0; bufIndex < NUMBER_OF_BUFFERS; ++bufIndex) {
        ...
        CHK_FATAL_HRESULT(m_pDevice->CreateShaderResourceView(
                              m_pDataBuffer[bufIndex],
                              &srvDesc,
                              &m_pDataBufferSRV[bufIndex]));
        ...
        CHK_FATAL_HRESULT(m_pDevice->CreateUnorderedAccessView(
                              m_pDataBuffer[bufIndex],
                              &uavDesc,
                              &m_pDataBufferUAV[bufIndex]));
...
void gameOfLife3D::lifeSimulator::LifeSimulatorCS::Copy(UINT index, OUT gameOfLife3D::lifeSimulator::LifeData *pLifeData)
{
...
HRESULT gameOfLife3D::lifeSimulator::LifeSimulatorCS::LoadRuleFile(
    ...
    OUT std::wstring &errorMessage)
{
    ...
    CComPtr<ID3DBlob> pCSBlob = nullptr;
    LPCSTR pProfile = ( m_pDevice->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0 ) ? "cs_5_0" : "cs_4_0";
    CHK_ERROR_HRESULT(hr, pD3DInteropHelper->CompileShaderFromFile(
                          (WCHAR*)fileName.c_str(),
                          "main",
                          pProfile,
                          &pCSBlob,
                          &errorMessage));
    ...
    CHK_ERROR_HRESULT(hr, m_pDevice->CreateComputeShader(
                          pCSBlob->GetBufferPointer(),
                          pCSBlob->GetBufferSize(),
                          nullptr,
                          &pComputeShader));
