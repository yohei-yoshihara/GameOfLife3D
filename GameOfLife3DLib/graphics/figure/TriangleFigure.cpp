...
HRESULT graphics::figure::TriangleFigure::Initialize(
    graphics::D3DInteropHelper *pD3DInteropHelper )
{
    ...
    // ジオメトリの初期化
    m_pGeometry = nullptr;
    CHK_FATAL_HRESULT(_CreateTriangleGeometry(
                          pD3DInteropHelper->GetD2DFactory(),
...
HRESULT graphics::figure::TriangleFigure::CreateDeviceDependentResources(
    graphics::D3DInteropHelper *pD3DInteropHelper,
...
HRESULT graphics::figure::TriangleFigure::Render(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget,
    FLOAT opacity)
{
...
void graphics::figure::TriangleFigure::SetColor(
...
HRESULT graphics::figure::TriangleFigure::_CreateTriangleGeometry(
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            case TRIANGLE_DIRECTION_LEFT:
                sink->BeginFigure(
                    D2D1::Point2F(rect.left, (rect.top + rect.bottom) / 2.0f),
            ...
            case TRIANGLE_DIRECTION_TOP:
                sink->BeginFigure(
                    D2D1::Point2F((rect.left + rect.right) / 2.0f, rect.top),
                    D2D1_FIGURE_BEGIN_FILLED
                );
            ...
            case TRIANGLE_DIRECTION_RIGHT:
                sink->BeginFigure(
                    D2D1::Point2F(rect.right, (rect.top + rect.bottom) / 2.0f),
                    D2D1_FIGURE_BEGIN_FILLED
                );
                sink->AddLine(D2D1::Point2F(rect.left, rect.bottom));
            ...
            case TRIANGLE_DIRECTION_BOTTOM:
                sink->BeginFigure(
                    D2D1::Point2F((rect.left + rect.right) / 2.0f, rect.bottom),
                    D2D1_FIGURE_BEGIN_FILLED
                );
                sink->AddLine(D2D1::Point2F(rect.left, rect.top));
                sink->AddLine(D2D1::Point2F(rect.right, rect.top));
