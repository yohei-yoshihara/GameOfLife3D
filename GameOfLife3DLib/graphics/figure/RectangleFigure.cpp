...
// #define DEBUG_RECTANGLE_FIGURE

graphics::figure::RectangleFigure::RectangleFigure(void)
    ...
    , m_glowColor(std::make_shared<graphics::color::SolidColor>(
          graphics::color::ColorValue(graphics::color::ColorValue::COLOR_TYPE_RGBA, 1.0f, 1.0f, 1.0f, 1.0f)))
    , m_decorators()
    , m_geometryFilter() {
...
HRESULT graphics::figure::RectangleFigure::Initialize(graphics::D3DInteropHelper *pD3DInteropHelper) {
#ifdef DEBUG_RECTANGLE_FIGURE
  LOG_ENTER(SEVERITY_LEVEL_DEBUG);
#endif
  HRESULT hr = S_OK;
  ...
  if (m_brushes->GetNumberOfBrushes() == 0) {
    SetColor(m_color, graphics::color::COLOR_PATTERN_GRADIENT);
  }

  // ジオメトリの初期化
  m_pGeometry = nullptr;
  CHK_FATAL_HRESULT(pD3DInteropHelper->GetD2DFactory()->CreateRoundedRectangleGeometry(
      D2D1::RoundedRect(D2D1::RectF(0.0f, 0.0f, m_width, m_height), m_radius, m_radius), &m_pGeometry));
  ...
  for (size_t i = 0; i < m_brushes->GetNumberOfBrushes(); ++i) {
    ...
    m_frames[i] = nullptr;
    CHK_FATAL_HRESULT(pD3DInteropHelper->GetD2DFactory()->CreateRoundedRectangleGeometry(
        D2D1::RoundedRect(D2D1::RectF(offset, offset, m_width - offset, m_height - offset), m_radius - offset,
                          m_radius - offset),
        &m_frames[i]));
  }
  ...
  // グロー効果の初期化
  if (m_glow && !HasGeometryFilter(L"#glow")) {
    AddGeometryFilter(L"#glow", L"InnerGaussFilter", FILTER_DISPLAY_ORDER_AFTER_FIGURE, m_glowColor);
  }
...
HRESULT graphics::figure::RectangleFigure::CreateDeviceDependentResources(graphics::D3DInteropHelper *pD3DInteropHelper,
...
HRESULT graphics::figure::RectangleFigure::Render(graphics::D3DInteropHelper *pD3DInteropHelper,
                                                  ID2D1RenderTarget *pRenderTarget, FLOAT opacity) {
#ifdef DEBUG_RECTANGLE_FIGURE
  LOG_ENTER(SEVERITY_LEVEL_DEBUG) << L"propertyChanged = " << std::boolalpha << m_propertyChanged;
#endif
...
void graphics::figure::RectangleFigure::DiscardDeviceDependentResources() {
#ifdef DEBUG_RECTANGLE_FIGURE
  LOG_ENTER(SEVERITY_LEVEL_DEBUG);
#endif
...
void graphics::figure::RectangleFigure::SetColor(const graphics::color::ColorValue &colorValue,
