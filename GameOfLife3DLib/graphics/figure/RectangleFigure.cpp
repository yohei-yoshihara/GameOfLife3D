...
HRESULT graphics::figure::RectangleFigure::Initialize(
    graphics::D3DInteropHelper *pD3DInteropHelper)
{
    ...
    for (auto it = m_geometryFilter.begin(); it != m_geometryFilter.end(); ++it) {
        ...
        LOG(SEVERITY_LEVEL_DEBUG) << L"filterName = " << geometryFilterWorkSet.GetFilterName()
                                  << L", filter = " << std::hex << filter.get() << std::dec;
#endif
...
HRESULT graphics::figure::RectangleFigure::CreateDeviceDependentResources(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget)
{
...
#endif
    HRESULT hr = S_OK;
    m_brushes->CreateDeviceDependentResource(
        pRenderTarget,
    ...
    for (auto it = m_geometryFilter.begin(); it != m_geometryFilter.end(); ++it) {
...
#ifdef DEBUG_RECTANGLE_FIGURE
        LOG(SEVERITY_LEVEL_DEBUG) << L"filterName = " << geometryFilterWorkSet.GetFilterName()
                                  << L", brush = " << std::hex << brush.get() << std::dec;
#endif
        ...
        CHK_FATAL_HRESULT(geometryFilter->ApplyFilter(
                              ...
                              GetWidth(),
                              GetHeight(),
                              geometryFilterWorkSet.GetBrush()->GetBrush(),
                              &bitmap,
                              &bounds));
...
#ifdef DEBUG_RECTANGLE_FIGURE
        LOG(SEVERITY_LEVEL_DEBUG) << L"bounds = " << bounds.left << L"," << bounds.top << L","
                                  << bounds.right << L"," << bounds.bottom;
#endif
    }
...
HRESULT graphics::figure::RectangleFigure::Render(
    graphics::D3DInteropHelper *pD3DInteropHelper,
    ID2D1RenderTarget *pRenderTarget,
    FLOAT opacity)
{
    ...
    for (auto it = m_geometryFilter.begin(); it != m_geometryFilter.end(); ++it) {
        ...
        if (geometryFilterWorkSet.GetFilterDisplayOrder() == FILTER_DISPLAY_ORDER_BEFORE_FIGURE) {
            pRenderTarget->DrawBitmap(
                geometryFilterWorkSet.GetBitmap(),
                geometryFilterWorkSet.GetEffectBounds(),
                opacity);
        }
    }
    for (size_t i = 0; i < m_brushes->GetNumberOfBrushes(); ++i) {
...
void graphics::figure::RectangleFigure::SetColor(
