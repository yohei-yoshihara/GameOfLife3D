...
HRESULT graphics::filter::GaussFilter::ApplyFilter(
...
//     pD3DInteropHelper->PushStates();
    ...
    if (SUCCEEDED(hr) && m_pGaussEffect == nullptr) {
        // Load pixel shader
        hr = pD3DInteropHelper->LoadD3D10ResourceShader(
                 pDevice,
    ...
    if (SUCCEEDED(hr)) {
        hr = pD3DInteropHelper->CreateD3D10Texture2D(
                 sourceTextureDesc.Width,
    ...
    OUT D2D1_RECT_F *pBitmapRect )
{
    ...
    CHK_FATAL_HRESULT(pD3DInteropHelper->CreateD3D10Texture2D(
                          textureWidth,
                          textureHeight,
                          &sourceTexture));

    CComPtr<ID2D1RenderTarget> textureRenderTarget = nullptr;
    CHK_FATAL_HRESULT(pD3DInteropHelper->CreateD2DRenderTargetFromD3D10Texture2D(
                          sourceTexture,
                          &textureRenderTarget));
    ...
    CHK_FATAL_HRESULT(pD3DInteropHelper->CreateD2DSharedBitmapFromD3D10Texture2D(
                          pRenderTarget,
                          destinationTexture,
                          ppBitmap));
...
HRESULT graphics::filter::GaussFilter::CreateVertexAndIndexBufferForFilter(
    graphics::D3DInteropHelper *pD3DInteropHelper)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = pD3DInteropHelper->GetD3D10Device()->CreateInputLayout(
                 graphics::filter::FilterCommon::INPUT_LAYOUT,
                 numElements,
...
void graphics::filter::GaussFilter::SetVertexAndIndexBufferForFilter(graphics::D3DInteropHelper *pD3DInteropHelper)
{
    ...
    pD3DInteropHelper->GetD3D10Device()->IASetVertexBuffers(
        0, // StartSlot
        1, // NumBuffers
        &pVertexBuffer,
        &stride,
