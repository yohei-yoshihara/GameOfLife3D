...
HRESULT graphics::filter::InnerGaussFilter::ApplyFilter(IN graphics::D3DInteropHelper *pD3DInteropHelper,
                                                        IN ID2D1RenderTarget *pRenderTarget,
                                                        IN ID2D1Geometry *pGeometry, IN FLOAT width, IN FLOAT height,
                                                        IN ID2D1Brush *pBrush, OUT ID2D1Bitmap **ppBitmap,
  ...
  CHK_ERROR_HRESULT(hr,
                    pD3DInteropHelper->GetD2DFactory()->CreateRectangleGeometry(
                        D2D1::RectF(0.0f, 0.0f, static_cast<FLOAT>(textureWidth), static_cast<FLOAT>(textureHeight)),
                        &outerRectangle));

  CComPtr<ID2D1PathGeometry> reversedGeometry = nullptr;
  CHK_ERROR_HRESULT(hr, pD3DInteropHelper->GetD2DFactory()->CreatePathGeometry(&reversedGeometry));

  CComPtr<ID2D1GeometrySink> geometrySink = nullptr;
  CHK_ERROR_HRESULT(hr, reversedGeometry->Open(&geometrySink));

  CHK_ERROR_HRESULT(hr, outerRectangle->CombineWithGeometry(
                            pGeometry, D2D1_COMBINE_MODE_EXCLUDE,
                            D2D1::Matrix3x2F::Translation(static_cast<FLOAT>(offset), static_cast<FLOAT>(offset)),
                            geometrySink));

  CHK_ERROR_HRESULT(hr, geometrySink->Close());

  CComPtr<ID3D10Texture2D> sourceTexture = nullptr;
  CHK_ERROR_HRESULT(hr, pD3DInteropHelper->CreateD3D10Texture2D(textureWidth, textureHeight, &sourceTexture));

  CComPtr<ID2D1RenderTarget> renderTarget = nullptr;
  CHK_ERROR_HRESULT(hr, pD3DInteropHelper->CreateD2DRenderTargetFromD3D10Texture2D(sourceTexture, &renderTarget));
  ...
  CHK_ERROR_HRESULT(hr, m_gaussFilter->ApplyFilter(pD3DInteropHelper, sourceTexture, &outputTexture));
  CComPtr<ID2D1Bitmap> outputTextureSB = nullptr;
  CHK_ERROR_HRESULT(
      hr, pD3DInteropHelper->CreateD2DSharedBitmapFromD3D10Texture2D(pRenderTarget, outputTexture, &outputTextureSB));
  ...
  D2D1_RECT_U srcRect
      = D2D1::RectU(offset, offset, static_cast<UINT32>(width + offset), static_cast<UINT32>(height + offset));
  CHK_ERROR_HRESULT(hr, resizedBitmap->CopyFromBitmap(&destPoint, outputTextureSB, &srcRect));

  CComPtr<ID2D1BitmapBrush> resizedBitmapBrush = nullptr;
  CHK_ERROR_HRESULT(hr,
                    pRenderTarget->CreateBitmapBrush(resizedBitmap, D2D1::BitmapBrushProperties(D2D1_EXTEND_MODE_CLAMP),
                                                     D2D1::BrushProperties(), &resizedBitmapBrush));

  CComPtr<ID3D10Texture2D> bitmapTexture = nullptr;
  CHK_ERROR_HRESULT(
      hr, pD3DInteropHelper->CreateD3D10Texture2D(static_cast<UINT>(width), static_cast<UINT>(height), &bitmapTexture));

  CComPtr<ID2D1RenderTarget> bitmapTextureRT = nullptr;
  CHK_ERROR_HRESULT(hr, pD3DInteropHelper->CreateD2DRenderTargetFromD3D10Texture2D(bitmapTexture, &bitmapTextureRT));
...
void graphics::filter::InnerGaussFilter::CalculateMargins(IN FLOAT width, IN FLOAT height, OUT FLOAT &leftMargin,
                                                          OUT FLOAT &topMargin, OUT FLOAT &rightMargin,
