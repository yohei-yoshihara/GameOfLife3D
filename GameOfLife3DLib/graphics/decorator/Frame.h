...
public:
    ...
    friend std::wostream& operator<<(std::wostream& os, const graphics::decorator::Frame &frame) {
        ...
        case graphics::decorator::Frame::BRUSH_REFERENCE_TYPE_BY_INDEX:
            os << typeid(frame).name() << L"[brushReferenceType=byIndex,brushIndex="
               << frame.m_brushIndex << L",thick=" << frame.m_thick << L"]";
            break;
        case graphics::decorator::Frame::BRUSH_REFERENCE_TYPE_BY_LABEL:
            os << typeid(frame).name() << L"[brushReferenceType=byLabel,brushLabel="
               << frame.m_brushLabel << L",thick=" << frame.m_thick << L"]";
            break;
        }
        return os;
    }
