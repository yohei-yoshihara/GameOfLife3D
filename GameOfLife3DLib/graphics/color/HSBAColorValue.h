  ...
  namespace color {
    ...
    public:
      ...
      bool operator==(const HSBAColorValue &other) const {
        return m_hue == other.m_hue && m_saturation == other.m_saturation && m_brightness == other.m_brightness
               && m_alpha == other.m_alpha;
      }
      bool operator!=(const HSBAColorValue &other) const {
        return !(*this == other);
      ...
      friend std::wostream &operator<<(std::wostream &os, const HSBAColorValue &hsbaColorValue) {
        os << typeid(hsbaColorValue).name() << L"[hue=" << hsbaColorValue.GetHue() << L",saturation="
           << hsbaColorValue.GetSaturation() << L",brightness=" << hsbaColorValue.GetBrightness() << L",alpha="
           << hsbaColorValue.GetAlpha() << L"]";
        return os;
      }
    };
  }
