...
HRESULT graphics::brush::RadialGradientBrush::CreateDeviceDependentResource(ID2D1RenderTarget *pRenderTarget,
  ...
  m_brush = nullptr;
  CHK_FATAL_HRESULT(pRenderTarget->CreateRadialGradientBrush(
      D2D1::RadialGradientBrushProperties(D2D1::Point2F(0, 0), offset, radiusX, radiusY),
      D2D1::BrushProperties(opacity, D2D1::Matrix3x2F::Translation(center.x, center.y)), gradientStopCollection,
      &m_brush));
...
ID2D1Brush *graphics::brush::RadialGradientBrush::GetBrush(FLOAT opacity) {
  if (m_brush != nullptr) {
    m_brush->SetOpacity(opacity);
    return m_brush;
  } else {
...
void graphics::brush::RadialGradientBrush::SetGradientStops(const graphics::color::GradientStopCollection &collection) {
