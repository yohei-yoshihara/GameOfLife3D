...
/*static*/
const SHORT graphics::D3DInteropHelper::s_D2DIndexArrayForBlend[] = {3, 1, 0, 2, 1, 3};
...
HRESULT graphics::D3DInteropHelper::CreateDeviceResources(UINT adapterIndex) {
  ...
  if (!m_pD3D11Device) {
    ...
    for (size_t i = 0; i < ARRAYSIZE(d3dDriverTypeAttempt); ++i) {
      hr = _CreateD3DDevice11(pDXGIAdapter1, d3dDriverTypeAttempt[i], nDeviceFlags, &m_pD3D11Device,
    ...
    wchar_t *d3d10DriverTypeAttemptLabel[] = {
        L"D3D10_DRIVER_TYPE_HARDWARE", L"D3D10_DRIVER_TYPE_WARP",
    };
#ifdef _DEBUG
    nDeviceFlags = D3D10_CREATE_DEVICE_BGRA_SUPPORT | D3D10_RESOURCE_MISC_SHARED_KEYEDMUTEX | D3D10_CREATE_DEVICE_DEBUG;
#else
    nDeviceFlags = D3D10_CREATE_DEVICE_BGRA_SUPPORT | D3D10_RESOURCE_MISC_SHARED_KEYEDMUTEX;
#endif
    ...
    if (SUCCEEDED(hr)) {
      if (!m_pD3DInteropAdapter.expired()) {
        m_pD3DInteropAdapter.lock()->CreateD3DResources(this, m_pD3D11Device, m_pD3D11ImmediateContext);
      }
    }
    if (SUCCEEDED(hr)) {
      if (!m_windowlessMode) {
    ...
    if (SUCCEEDED(hr)) {
      if (!m_pD3DInteropAdapter.expired()) {
        m_pD3DInteropAdapter.lock()->CreateD2DResources(this, m_pD2DFactory, m_pD2DTexture2DRenderTargetD3D11);
      }
    }
  }
#ifdef DEBUG_GRAPHICS_D3DINTEROPHELPER_
...
HRESULT graphics::D3DInteropHelper::_RecreateSizedResources11(UINT nWidth, UINT nHeight) {
  ...
  if (SUCCEEDED(hr)) {
    if (!m_pD3DInteropAdapter.expired()) {
      m_pD3DInteropAdapter.lock()->RecreateSizedResources(nWidth, nHeight, this, m_pD3D11Device,
                                                          m_pD3D11ImmediateContext);
    }
  }
...
HRESULT graphics::D3DInteropHelper::_CreateD3DDevice11(IDXGIAdapter1 *pAdapter, D3D_DRIVER_TYPE driverType, UINT flags,
  ...
  hr = D3D11CreateDevice(pAdapter, driverType, nullptr, flags, levelAttempts, ARRAYSIZE(levelAttempts),
...
HRESULT graphics::D3DInteropHelper::_CreateD3DDevice10(IDXGIAdapter *pAdapter, D3D10_DRIVER_TYPE driverType, UINT flags,
  ...
  static const wchar_t *levelLabels[] = {
      L"D3D10_FEATURE_LEVEL_10_1", L"D3D10_FEATURE_LEVEL_10_0", L"D3D10_FEATURE_LEVEL_9_3",
  };
...
HRESULT graphics::D3DInteropHelper::CreateDeviceIndependentResources() {
  ...
  if (SUCCEEDED(hr) && !m_pD3DInteropAdapter.expired()) {
    hr = m_pD3DInteropAdapter.lock()->CreateDeviceIndependentResources(this, m_pD2DFactory, m_pWICFactory,
                                                                       m_pDWriteFactory);
  }
...
HRESULT graphics::D3DInteropHelper::CreateSharedSurface(UINT width, UINT height,
...
HRESULT graphics::D3DInteropHelper::Blend(ID3D11Texture2D *pTexture) {
  ...
  if (m_pVertexShaderForBlend == nullptr) {
    ...
    if (m_pVertexLayoutForBlend == nullptr) {
      hr = m_pD3D11Device->CreateInputLayout(s_D2DInputLayoutForBlend, ARRAYSIZE(s_D2DInputLayoutForBlend),
  ...
  m_pD3D11ImmediateContext->IASetVertexBuffers(0, // StartSlot
                                               1, // NumBuffers
...
HRESULT graphics::D3DInteropHelper::LoadResourceBitmap(ID2D1RenderTarget *pRenderTarget,
                                                       IWICImagingFactory *pIWICFactory, PCWSTR resourceName,
                                                       PCWSTR resourceType, UINT destinationWidth,
...
HRESULT graphics::D3DInteropHelper::LoadBitmapFile(ID2D1RenderTarget *pRenderTarget, IWICImagingFactory *pIWICFactory,
                                                   const wchar_t *fileName, ID2D1Bitmap **ppBitmap) {
  HRESULT hr = S_OK;

  CComPtr<IWICBitmapDecoder> decoder = nullptr;
  hr = pIWICFactory->CreateDecoderFromFilename(fileName, nullptr, GENERIC_READ, WICDecodeMetadataCacheOnDemand,
                                               &decoder);
  ...
  if (SUCCEEDED(hr)) {
    hr = formatConverter->Initialize(frame, GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone, nullptr, 0.0f,
...
HRESULT graphics::D3DInteropHelper::CompileShaderFromResource(ID3D10Device *pDevice, PCWSTR pszResource,
...
HRESULT graphics::D3DInteropHelper::CompileShaderFromResource(PCWSTR pszResource, LPCSTR szEntryPoint,
                                                              LPCSTR szShaderModel, OUT ID3DBlob **ppBlob,
  ...
  hr = D3DX11CompileFromResource(
      ...
      &pErrorBlob,
      nullptr);*/
  hr = D3DCompile(data.data(), size, "LoadResourceShader", nullptr, nullptr, szEntryPoint, szShaderModel, dwShaderFlags,
                  0, ppBlob, &pErrorBlob);
...
HRESULT graphics::D3DInteropHelper::CompileShaderFromFile(WCHAR *szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel,
                                                          OUT ID3DBlob **ppBlobOut, OUT std::wstring *errorMessage) {
  HRESULT hr = S_OK;
...
HRESULT graphics::D3DInteropHelper::_InitializeAnimation() {
  HRESULT hr
      = CoCreateInstance(CLSID_UIAnimationManager, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&m_pAnimationManager));
  if (SUCCEEDED(hr)) {
  ...
  if (SUCCEEDED(hr)) {
    hr = animation::AnimationManagerEventHandler::CreateInstance(m_animationWindow, &pAnimationManagerEventHandler);
  }
  if (SUCCEEDED(hr)) {
...
void graphics::D3DInteropHelper::PushMatrix(ID2D1RenderTarget *pRenderTarget, const D2D1_MATRIX_3X2_F &matrix) {
...
void graphics::D3DInteropHelper::RegisterGeometryFilter(
    const std::wstring &geometryFilterName, std::shared_ptr<graphics::filter::IGeometryFilter> geometryFilter) {
  if (m_geometryFilterMap.count(geometryFilterName) == 0) {
    m_geometryFilterMap.insert(std::pair<std::wstring, std::shared_ptr<graphics::filter::IGeometryFilter>>(
        geometryFilterName, geometryFilter));
  }
}

std::shared_ptr<graphics::filter::IGeometryFilter>
graphics::D3DInteropHelper::GetGeometryFilter(const std::wstring &geometryFilterName) const {
  return m_geometryFilterMap.at(geometryFilterName);
}

void graphics::D3DInteropHelper::RegisterTextureFilter(
    const std::wstring &textureFilterName, std::shared_ptr<graphics::filter::ITextureFilter> textureFilter) {
  if (m_textureFilterMap.count(textureFilterName) == 0) {
    m_textureFilterMap.insert(
        std::pair<std::wstring, std::shared_ptr<graphics::filter::ITextureFilter>>(textureFilterName, textureFilter));
  }
}

std::shared_ptr<graphics::filter::ITextureFilter>
graphics::D3DInteropHelper::GetTextureFilter(const std::wstring &textureFilterName) const {
...
HRESULT graphics::D3DInteropHelper::DetectMSAA() {
  ...
  for (size_t i = 0; i < ARRAYSIZE(sampleCounts); ++i) {
    ...
    HRESULT hr
        = m_pD3D11Device->CheckMultisampleQualityLevels(DXGI_FORMAT_B8G8R8A8_UNORM, sampleCounts[i], &numQualityLevels);
    if (FAILED(hr)) {
      break;
    }
...
HRESULT graphics::D3DInteropHelper::LoadD3D10ResourceShader(ID3D10Device *pDevice, PCWSTR pszResource,
                                                            ID3D10Effect **ppShader) {
  HRESULT hr;
...
HRESULT graphics::D3DInteropHelper::CreateD3D10Texture2D(UINT width, UINT height, OUT ID3D10Texture2D **ppTexture2D) {
  assert(width > 0);
  assert(height > 0);
...
HRESULT graphics::D3DInteropHelper::CreateD2DRenderTargetFromD3D10Texture2D(ID3D10Texture2D *pTexure2D,
...
HRESULT graphics::D3DInteropHelper::CreateD2DSharedBitmapFromD3D10Texture2D(ID2D1RenderTarget *pRenderTarget,
...
HRESULT graphics::D3DInteropHelper::_CreateCSDevice(IDXGIAdapter1 *pAdapter, UINT flags, OUT ID3D11Device **ppDevice,
  ...
  if (m_pD3D11Device->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0
    ...
    *ppImmediateContext = m_pD3D11ImmediateContext;
    ...
    hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_REFERENCE, NULL, uCreationFlags, flvl, ARRAYSIZE(flvl),
                           D3D11_SDK_VERSION, ppDevice, &flOut, ppImmediateContext);
