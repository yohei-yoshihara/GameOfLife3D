...
tsf::TSFManager::TSFManager(void)
    : m_hAccel(nullptr)
    , m_hWnd(nullptr)
...
void tsf::TSFManager::Initialize() {
  HRESULT hr
...
int tsf::TSFManager::RunMessageLoop() {
  int nReturn = 0;
  BOOL fResult = TRUE;
...
HRESULT tsf::TSFManager::ActivateThreadMgr() {
  HRESULT hr = S_OK;
...
HRESULT tsf::TSFManager::DeactivateThreadMagr() {
  HRESULT hr = S_OK;
  if (m_isThreadMgrActivated) {
